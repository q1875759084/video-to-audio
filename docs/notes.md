...
**背景**：需要支持用户指定起止时间，只提取视频中某个时间段的音频，或同时指定多个片段并合并输出。

**工具能力**：yt-dlp 和 ffmpeg 均原生支持时间裁剪，关键在于组合方式。

#### 方案对比

| 方案 | 流程 | 带宽效率 | 适用场景 |
|------|------|---------|---------|
| 方案 A：yt-dlp 完整下载 + ffmpeg 本地裁剪 | 先下载全部，再裁剪 | 低（下载完整视频） | 简单，但长视频浪费 |
| 方案 B：yt-dlp `--download-sections` | yt-dlp 直接只下载指定段 | 高 | CDN 支持 Range 时推荐 |
| **方案 C（当前采用）**：yt-dlp -g 获取直链 + ffmpeg 裁剪直连 | 代理获取 CDN 直链，ffmpeg 直接对直链做裁剪下载 | 最高 | 复用两步走架构，天然适合 |

**当前项目采用方案 C**，理由：项目已有「yt-dlp -g → wget 下载」的两步走架构，只需将 wget 替换为带 `-ss/-to` 参数的 ffmpeg 命令，即可实现裁剪下载，无需额外引入新工具或改变整体流程。

#### 方案 C 的执行流程

```
1. yt-dlp -g --proxy socks5h://... URL
   → 通过代理获取 CDN 直链（<1s，只是 API 调用）

2. ffmpeg -ss 00:01:30 -to 00:03:00 -i "https://cdn.bilibili.com/...直链..." -vn output.mp3
   → ffmpeg 以 CDN URL 为输入，内部发 HTTP Range 请求，只取对应时间段的字节
   → 直连服务器带宽，不走代理
```

第二步中，ffmpeg 本身内置 HTTP 客户端，输入为 URL 时自动处理网络请求，行为与本地文件完全透明，上层只需传时间参数。

---

### HTTP Range 在不同场景的使用边界

HTTP Range 请求（`Range: bytes=start-end`）是 HTTP/1.1 标准头，用于**请求资源的某个字节范围**，而非语义层面的"第几分钟"。实际开发中有三种使用场景，职责各不相同：

#### 场景一：音视频播放器拖进度条（浏览器自动，无需手写）

```
用户拖动 <audio> 进度条
  → 浏览器自动计算时间对应的字节偏移
  → 自动发送 Range: bytes=1048576-2097152
  → 后端返回 206 Partial Content
```

前后端开发者的工作：
- **前端**：不需要做任何事，`<audio src="...">` 浏览器自动处理
- **后端**：正确响应 `206 Partial Content`，返回 `Content-Range` 和 `Accept-Ranges` 头（当前 `/preview` 接口已实现）

#### 场景二：时间裁剪转换（业务语义，不用 Range 头）

```
用户输入：起点 00:01:30，终点 00:03:00
  → 前端 POST body: { start: "00:01:30", end: "00:03:00" }
  → 后端拼 ffmpeg 参数：-ss 00:01:30 -to 00:03:00
  → ffmpeg 内部算字节偏移，对 CDN URL 发 Range 请求
```

这里 HTTP Range 是 ffmpeg 内部实现细节，对前后端完全透明。前端传**时间码**（人类可读），后端直接透传给 ffmpeg，无需自己做时间→字节的换算。

**规则**：凡是有专门工具处理语义（ffmpeg 处理时间、数据库处理 OFFSET/LIMIT），就用业务参数，不要在 HTTP 层手动算字节偏移。

#### 场景三：断点续传/分片下载（需要手动处理字节偏移）

```
客户端已下载 1MB，继续下载剩余部分：
  → 客户端发 Range: bytes=1048576-
  → 服务端返回 206，从该偏移继续输出
```

这种场景才需要前端显式计算和携带字节范围，用于大文件下载断点续传、多线程并发分片下载等场景。当前项目暂不涉及。

---

### 多片段合并策略

当用户指定多个时间片段时，有两种输出策略：

**策略 A：合并为单文件（当前采用）**

```
片段1（00:01:30~00:03:00）→ segment_0.mp3
片段2（00:05:00~00:07:30）→ segment_1.mp3
           ↓ ffmpeg concat
         output.mp3（连续播放，两段无间隔）
```

实现方式：ffmpeg concat demuxer
```bash
# 生成 concat 列表文件
echo "file 'segment_0.mp3'" > concat.txt
echo "file 'segment_1.mp3'" >> concat.txt
# 合并（-c copy 不重新编码，极快）
ffmpeg -f concat -safe 0 -i concat.txt -c copy output.mp3
```

**策略 B：分别输出多个文件，打包 zip**（未来扩展）

适合用户需要独立片段的场景（如提取多首歌），需要后端打包并输出 zip，前端下载解压。
